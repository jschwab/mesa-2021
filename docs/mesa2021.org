...
** Part 1a: The modules
MESA (Modules for Experiments in Stellar Astrophysics) is composed of modules.  These are collections of Fortran that serve a common purpose.  When you list the contents of ~$MESA_DIR~, most of the subdirectories that you see are modules.
...
*** Task 1 (Example): Add a stopping condition
